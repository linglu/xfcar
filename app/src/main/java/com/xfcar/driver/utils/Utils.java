...
//        try {
//            message = SecurityUtils.genMd5(message.getBytes()).toLowerCase();
//        } catch (Exception ex) {
//        }
//        return message;
//    }
     ...
     * 检测是否覆盖安装的版本
     */
    public static boolean isNewVersion(Context context, int oldVersionCode) {
        int versionCode = getVersionCode(context);
        return oldVersionCode != 0 && versionCode > oldVersionCode;
    }
    ...
    public static void requestPermission(Activity activity, int requestCode, String[]... permissions) {
        if (isVersionM()) {
            ...
            for (String[] permissionGroup : permissions) {
                for (String permission : permissionGroup) {
                    if (!Utils.hasPermission(activity, permission)) {
